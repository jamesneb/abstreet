...
use map_model::{RoadID, IntersectionID, Traversable};
use map_gui::tools::{ColorNetwork, DivergingScale};
use map_gui::ID;
use widgetry::mapspace::ToggleZoomed;
use widgetry::{EventCtx, GfxCtx, Outcome, Panel, Toggle, Color, Text, Widget};
        ...
        if ctx.redo_mouseover() || recalc_tooltip {
            ...
            match app.mouseover_unzoomed_roads_and_intersections(ctx) {
                Some(ID::Road(r)) => {
                    ...
                    self.tooltip = Some(Text::from(format!("{} before, {} after", prettyprint_usize(before), prettyprint_usize(after))));
                }
                Some(ID::Intersection(i)) => {
                    let (before, after) = (self.before_intersection.get(i), self.after_intersection.get(i));
                    self.tooltip = Some(Text::from(format!("{} before, {} after", prettyprint_usize(before), prettyprint_usize(after))));
                }
                _ => {}
            }
...
impl RelativeProblemMap {
    pub fn new(ctx: &mut EventCtx, app: &App, opts: Options) -> Self {
        ...
        let update_count = |problem: &Problem, roads: &mut Counter<RoadID>, intersections: &mut Counter<IntersectionID>| {
            match problem {
        ...
        for (i, before, after) in before_intersection.clone().compare(after_intersection.clone()) {
            if let Some(c) = scale.eval((after as f64) / (before as f64)) {
                colorer.add_i(i, c);
            }
        }
